#Modified and ported By Saeed Desouky
...
adc_y = Pin(JOYSTICK_Y_PIN, Pin.IN)
sw = Pin(JOYSTICK_SW_PIN, Pin.IN)
np = NeoPixel(Pin(13, Pin.OUT), 2); np.bright = 50

rand_seed = Pin(RAND_SEED_PIN, Pin.IN).value()
seed = 0
for _ in range(1000):
    seed += rand_seed
seed *= 10
...
def setLED(r, g, b):
    for i in range(np.n): np[i] = (int(r * np.bright / 100), int(g * np.bright / 100), int(b * np.bright / 100))
    np.write()
...
setLED(256, 256, 32)
utime.sleep(0.1)
displayCenterText('TekTech')
utime.sleep(2)
displayCenterText('HELLO')
utime.sleep(2)
displayCenterText('Lets Play')
utime.sleep(1)
...
def start ():
    ...
    global score
    global score_show
    global analog_input_center 
    global counter
    global score_show
    while True:
        while True:
            if score_show:
                display.text("PRESS",45,15,1)
                display.text("B2 TO START",20,28,1)
                display.show()
            else:
                displayClear()
            score_show = not score_show
