...
//static const int SPI_Frequency = 1E6;
...
void lcdInit(TFT_t * dev, uint16_t model, int width, int height, int offsetx, int offsety)
{
	...
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735 || dev->_model == 0x7735) {
	...
	if (dev->_model == 0x7789) {
		...
		spi_master_write_comm_byte(dev, 0x3A); // Set color mode
		spi_master_write_data_byte(dev, 0x55); // 16-bit color

		spi_master_write_comm_byte(dev, 0x36); // Mem access ctrl (directions)
		// spi_master_write_data_byte(dev, 0x08);
		spi_master_write_data_byte(dev, 0xA0);
		/*
		spi_master_write_comm_byte(dev, 0x2A); // Column addr set
		spi_master_write_data_byte(dev, 0x00);
		spi_master_write_data_byte(dev, 0);
		spi_master_write_data_byte(dev, 0);
		spi_master_write_data_byte(dev, 240);
		
		spi_master_write_comm_byte(dev, 0x2B); // Row addr set
		spi_master_write_data_byte(dev, 0x00);
		spi_master_write_data_byte(dev, 0);
		spi_master_write_data_byte(dev, 320>>8);
		spi_master_write_data_byte(dev, 320&0xFF);
*/
		spi_master_write_comm_byte(dev, 0x21); // hack
		spi_master_write_comm_byte(dev, 0x13); // Normal display on

		spi_master_write_comm_byte(dev, 0x11); // Out of sleep mode,
		delayMS(10);

		spi_master_write_comm_byte(dev, 0x29); // Main screen turn on
	} // endif 0x7789
...
void lcdDrawPixel(TFT_t * dev, uint16_t x, uint16_t y, uint16_t color){
	...
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7796 || dev->_model == 0x7789) {
	...
	} // endif 0x9340/0x9341/0x7796/0x7789
...
void lcdDrawArrow(TFT_t * dev, uint16_t x0,uint16_t y0,uint16_t x1,uint16_t y1,uint16_t w,uint16_t color) {
	...
	uint16_t L[2],R[2];
	L[0]= (uint16_t)(x1 - Uy*w - Ux*v);
	L[1]= (uint16_t)(y1 + Ux*w - Uy*v);
	R[0]= (uint16_t)(x1 + Uy*w - Ux*v);
	R[1]= (uint16_t)(y1 - Ux*w - Uy*v);
	//   printf("L=%d-%d R=%d-%d\n",L[0],L[1],R[0],R[1]);
...
void lcdDrawFillArrow(TFT_t * dev, uint16_t x0,uint16_t y0,uint16_t x1,uint16_t y1,uint16_t w,uint16_t color) {
	...
	int ww;
	for(ww=w-1;ww>0;ww--) {
		L[0]= (uint16_t)(x1 - Uy*ww - Ux*v);
		L[1]= (uint16_t)(y1 + Ux*ww - Uy*v);
		R[0]= (uint16_t)(x1 + Uy*ww - Ux*v);
		R[1]= (uint16_t)(y1 - Ux*ww - Uy*v);
